...
---@field job fun(task: zulTask, date: osdate)
---@field isActive boolean
---@field id number
---@field debug? boolean

---@class zulTask : zulTaskProperties
---@field private scheduleTask fun(self: zulTask): boolean?
local zulTask = {}
zulTask.__index = zulTask
...
---@return number
function zulTask:getAbsoluteNextTime()
    local minute = getTimeUnit(self.minute, 'min')
...
---@param job fun(task: zulTask, date: osdate)
...
function zlib.cron.new(expression, job, options)
    ...
    ---@type zulTask
    local task = setmetatable(options or {}, zulTask)
