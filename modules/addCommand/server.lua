...
---@param ... any
function zlib.addCommand(commandName, properties, cb, ...)
    ...
    if properties then
        if ... or table.type(properties) ~= 'hash' then
            ...
            warn(("command '%s' is using deprecated syntax for zlib.addCommand\nupdate the command or use zlib.__addCommand to ignore this warning\n> source ^0(^5%s^0:%d)"):format(_commandName, info.short_src, info.currentline))
            ---@diagnostic disable-next-line: deprecated
            return zlib.__addCommand(commandName, properties, cb, ...)
        end
...
return zlib.addCommand

--[[
    zlib.addCommand('giveitem', {
